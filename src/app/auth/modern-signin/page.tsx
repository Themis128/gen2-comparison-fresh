...
import type { SignInResult } from "@/lib/useAuth";
...
export default function ModernSignInPage() {
  ...
  const handleSignIn = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Use USER_AUTH flow with PASSWORD_SRP preferred challenge for modern auth
      const result = await signInWithFlow(email, password, {
        authFlowType: "USER_AUTH" as AuthFlowType,
        preferredChallenge: "PASSWORD_SRP" as PreferredChallenge,
      });
      ...
      if (
        ...
        setError(`Authentication step: ${result.nextStep.signInStep}`);
      }
    } catch (err: unknown) {
  ...
  const handleChallengeSelection = async (challenge: string) => {
    setIsLoading(true);
    setError(null);

    try {
      const result = await confirmSignInWithChallenge(challenge);
      setSignInStep(result);
  ...
  const handleGoogleSignIn = async () => {
    setIsLoading(true);
    setError("Social sign-in not configured yet");
    setTimeout(() => {
      setIsLoading(false);
    }, 1000);
  };
              ...
              <div className="flex flex-col gap-6">
                <div className="flex flex-col items-center gap-2 text-center">
                  ...
                  <p className="text-muted-foreground text-sm text-balance">
                    Select how you&apos;d like to complete sign-in
                  </p>
                </div>
