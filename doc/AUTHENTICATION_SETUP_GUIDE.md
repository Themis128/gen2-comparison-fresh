...
This guide provides step-by-step instructions to configure and test the complete authentication system for the Fresh Gen2 Amplify Todo App built with **Next.js 15**, **React 18.3.0**, and **TypeScript 5.9.3**.
...
### 5. Test Basic Auth
Visit `http://localhost:3001` - should redirect to sign-in page.
...
3. **Configure Authorization Callback URL**
   ```
   Authorization callback URL: http://localhost:3001/auth/signin
   ```
...
```json
{
  "auth": {
    ...
    "oauth": {
      "redirect_sign_in_uri": ["http://localhost:3001/auth/signin"],
      "redirect_sign_out_uri": ["http://localhost:3001/auth/signin"]
    }
  }
}
```
...
8. Should access todo dashboard at `/app`
...
2. Try to visit `http://localhost:3001/app` directly
...
4. Verify OAuth app redirect URIs match exactly (e.g., `http://localhost:3001/auth/signin`)
...
2. Ensure `http://localhost:3001/auth/signin` is in allowed redirect URIs
