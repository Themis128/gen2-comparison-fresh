...
test.describe('Authentication Flow Tests', () => {
  test('should load home page in test mode without redirect', async ({
    ...
    await expect(
      page.locator('text=AWS Amplify Gen2 Authentication Demo')
    ).toBeVisible();
  });
  ...
  test('should load sign-in page correctly', async ({ page }) => {
    ...
    await expect(
      page.locator('text=Sign in to your account to continue')
    ).toBeVisible();
  ...
  test('should load sign-up page correctly', async ({ page }) => {
    ...
    await expect(
      page.locator('text=Join us today and get started')
    ).toBeVisible();
  ...
  test('should load forgot-password page correctly', async ({ page }) => {
    ...
    // Check page title and content
    await expect(
      page.locator('h1').filter({ hasText: 'Reset your password' })
    ).toBeVisible();
    await expect(
      page.locator(
    ...
    await expect(page.locator('input[type="email"]')).toBeVisible();
    await expect(
      page.locator('button:has-text("Send reset email")')
    ).toBeVisible();
  });
  ...
  test('should load test page without authentication', async ({ page }) => {
    ...
    await expect(
      page
        .locator('h1')
        .filter({ hasText: 'Fresh Gen2 Amplify App - Test Mode' })
    ).toBeVisible();
  });
});
